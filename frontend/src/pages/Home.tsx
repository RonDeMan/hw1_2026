import { useCallback, useEffect, useState, useReducer, useRef, useContext } from 'react'
import '../App.css'
      ...
      <div style={{ marginBottom: '20px' }}>
        {authContext?.user ? (
          <div>
            ...
            <button data-testid="logout" onClick={() => authContext.logout()} style={{ marginLeft: '10px' }}>Logout</button>
          </div>
        ) : (
          <div>
            <button data-testid="go_to_login_button" onClick={() => navigate('/login')}>Go to Login</button>
            <button data-testid="go_to_create_user_button" onClick={() => navigate('/create-user')} style={{ marginLeft: '10px' }}>Create New User</button>
          </div>
        )}
      </div>
